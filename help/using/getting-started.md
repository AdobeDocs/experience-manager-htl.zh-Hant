...
HTML範本語言(HTL)是Adobe Experience Manager中首選且推薦使用的HTML伺服器端範本系統。 因為在所有 HTML 伺服器端範本系統中，HTL 檔案會指定 HTML 本身的內容、一些基本的表現邏輯，以及要在執行階段運算的變數，藉此定義傳送給瀏覽器的輸出。
...
HTL 的核心概念是提供重複使用現有 HTML 元素，來定義區塊陳述式的可能性。這種重複使用方法可避免插入額外的分隔符號來定義陳述式的開始和結束位置。對標記進行註解可以將靜態 HTML 低調地轉換為動態範本，而不破壞 HTML 的有效性，進而確保即使作為靜態檔案也能正確顯示。

不過，有時在必須插入區塊陳述式的確切位置可能沒有現有元素。在這種情況下，您可以插入特殊的 `sly` 元素。當執行附加的區塊陳述式並相應地顯示其內容時，該元素會自動從輸出中移除。
...
輸出類似下列 HTML 的內容，但前提是 `jcr:title` 和 `jcr:description` 屬性均有定義，而且兩者都不是空白：
...
請記得一件事，唯有沒有任何現有元素可能使用此區塊元素作為附註時，才能使用 `sly` 元素。原因是 `sly` 元素會阻止此語言所提供的值，以便將 HTML 變成動態時避免更動靜態 HTML。
...
如同底下[自動上下文感知逸出](#automatic-context-aware-escaping)一節中所述，HTL 的一個目的是藉由自動套用上下文感知逸出到所有運算式來減少引進跨網站指令碼 (XSS) 安全漏洞的風險。 HTL 會偵測 HTML 標記中運算式的上下文，但不分析內嵌 JavaScript 或 CSS，因此開發人員必須指定這些運算式的確切上下文。
...
本節會介紹一些常見場景。其說明如何使用 HTML 範本語言，以最佳方法來解決這些場景。
...
以下範例示範如何將物件序列化為 JSON (也可以在 Java 中)，以傳遞給用戶端。然後可以輕鬆地將其放入 `data` 屬性中：
...
`script` 元素的標記可以包含沒有明確內容的 HTL 區塊陳述式，因為 Handlebars 範本內容隔離在自己的檔案中。此外，這個範例也說明如何將伺服器端執行的 HTL (例如在 `h2` 元素上) 與用戶端執行的範本語言 (例如顯示在 `h3` 元素上的 Handlebars) 混合在一起。

不過，一種更新型的技巧是改用 HTML `template` 元素，因為這樣做有個好處：不需要將範本的內容隔離到個別檔案中。

### 解除特殊設定語法的限制 {#lifting-limitations-of-special-contexts}

在需要略過指令碼、樣式和註解設定語法之限制的特殊情況下，可以將其內容隔離在另一個 HTL 檔案中。 HTL 會將自己檔案中的所有內容解釋為標準 HTML 片段，忽略包含它的任何限制上下文。

如需範例，請參閱下面的[使用用戶端範本](#working-with-client-side-templates)一節。

>[!CAUTION]
>
>此技術可能會引入跨網站指令碼 (XSS) 漏洞。如果使用這種方法，則應仔細研究安全性。通常會有更好的方法可以實施相同的事，而不必依賴此一做法。
