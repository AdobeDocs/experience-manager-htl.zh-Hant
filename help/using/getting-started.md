...
>[!NOTE]
>
>要運行本頁上提供的大多數示例，可以使用名為[讀取評估打印循環](https://github.com/Adobe-Marketing-Cloud/aem-htl-repl)的即時執行環境。
...
**HTL是否支援JSP標籤庫？** -否，但如「載入客戶機庫」 [部分所](getting-started.md#loading-client-libraries) 示，模板和 [callstatements提](block-statements.md#template-call) 供了類似的模式。

**AEM專案可擴充HTL功能嗎？** -不，他們不能。HTL具有強大的擴充機制，可重複使用邏輯- [Use-API](getting-started.md#use-api-for-accessing-logic) —— 和標籤（[template &amp; call](block-statements.md#template-call)陳述式），可用來模組化專案的程式碼。
...
* **[塊語句](block-statements.md)** -要有條件地顯示  **&lt;h1>** 元素，則 [`data-sly-test`](block-statements.md#test) 會使用HTML5資料屬性。HTL提供多種此類屬性，可將行為附加至任何HTML元素，而且所有屬性都加上前置詞`data-sly`。
...
運算式只能放入HTML文字或屬性值中，但不能放在元素名稱或屬性名稱中，否則將不再是有效的HTML。 為了動態設定元素名稱，[`data-sly-element`](block-statements.md#element)語句可用於所需的元素，並動態設定屬性名稱，即使一次設定多個屬性，也可以使用[`data-sly-attribute`](block-statements.md#attribute)語句。
...
這聽起來可能像是一個很大的限制，但是它是需要的，因為HTML範本語言不應被濫用來產生非HTML的輸出。 下面的[存取邏輯的使用-API一節介紹如何從範本呼叫其他邏輯，如果需要範本來準備複雜輸出，則可使用範本。 ](getting-started.md#use-api-for-accessing-logic)例如，從後端傳送資料至前端指令碼的簡單方式是讓元件的邏輯產生JSON字串，然後再將它放入資料屬性中，並使用簡單的HTL運算式。
...
如下方[「自動內容感知逸出」一節所述，HTL的目標之一是透過自動將內容感知逸出套用至所有運算式，降低引入跨網站指令碼(XSS)弱點的風險。 ](getting-started.md#automatic-context-aware-escaping)雖然HTL可自動偵測置於HTML標籤內之運算式的上下文，但它不會分析內嵌JavaScript或CSS的語法，因此需仰賴開發人員明確指定應套用至這些運算式的確切上下文。
...
有關如何控制逸出的詳細資訊，請參閱[表達式語言顯示上下文](expression-language.md#display-context)部分。
...
如需範例，請參閱[使用用戶端範本](getting-started.md#working-with-client-side-templates)一節。
...
在伺服器端執行的`logic.js` JavaScript檔案旁邊放置：
...
* [`data-sly-use`語句](block-statements.md#use)上的部分說明了可使用該語句執行的所有操作。
* [Use-API頁面](use-api.md)提供一些資訊，以協助您選擇使用Java或JavaScript編寫邏輯。
* 若要詳細說明如何編寫邏輯，[JavaScript Use-API](use-api-javascript.md)和[Java Use-API](use-api-java.md)頁面應有幫助。

### 自動上下文感應逸出{#automatic-context-aware-escaping}
...
請注意，這兩個屬性的逸出方式不同，因為HTL知道`href`和`src`屬性必須逸出以用於URI內容。 此外，如果URI以&#x200B;**`javascript:`**&#x200B;開頭，則屬性將會完全移除，除非上下文已明確變更為其他內容。
...
對於設定屬性，[`data-sly-attribute`](block-statements.md#attribute)語句也可能很有用。

## 使用HTL {#common-patterns-with-htl}的常見模式

本節介紹一些常見的案例，以及如何使用HTML範本語言來最佳解決它們。

### 載入客戶端庫{#loading-client-libraries}

在HTL中，用戶端程式庫會透過AEM提供的輔助範本載入，此範本可透過[`data-sly-use`](block-statements.md#use)存取。 此檔案中有三個模板，可通過[`data-sly-call`](block-statements.md#template-call)調用：
...
[template &amp; call](block-statements.md#template-call)陳述式中的章節提供更多有關宣告和呼叫此類範本的詳細資訊。
...
### 使用客戶端模板{#working-with-client-side-templates}

其中，[Lifting Limitations of Special Contexts](getting-started.md#lifting-limitations-of-special-contexts)一節中說明的技術可合法使用的一個特殊情況是，編寫位於&#x200B;**script**&#x200B;元素內的用戶端範本（例如Handlebar）。 此技巧之所以可安全使用，是因為&#x200B;**script**&#x200B;元素不會像假設的那樣包含JavaScript，而會包含更多HTML元素。 以下是如何運作的範例：
...
* [塊語句](block-statements.md) -用於發現HTL中所有可用的塊語句，以及如何使用這些語句。
