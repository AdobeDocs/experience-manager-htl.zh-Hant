...
The HTML Template Language (HTL) Java Use-API enables an HTL file to access helper methods in a custom Java class through `data-sly-use`. 這可讓所有複雜的商業邏輯封裝在Java程式碼中，而HTL程式碼則僅處理直接標籤製作。
...
對 `bindings` 應可包含物件，這些物件會提供目前執行的HTL指令碼，Use-API物件可用來處理它。

## 簡單範例 {#a-simple-example}

我們將從沒有use-class的HTL元件開始。 它由一個檔案組成， `/apps/my-example/components/info.html`
...
### 本機與Bundle Java類 {#local-vs-bundle-java-class}

Java use-class可以用兩種方式安裝： **本機** 或 **搭售**。 此示例使用本地安裝。
...
在本例中， `Info.java` 位於的 `/apps/my-example/components/info` 位置是，因此軟體包為 `apps.my_example.components.info`:
...
### 延伸 `WCMUsePojo` {#extending-wcmusepojo}
...
從擴展use-class時，通過覆 `WCMUsePojo`蓋方法來執行初 `activate` 始化：
...
通常， [activate](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/cq/sightly/WCMUsePojo.html) 方法會用來根據目前的上下文（例如目前的請求和資源），預先計算並儲存（在成員變數中）HTL程式碼中所需的值。

類 `WCMUsePojo` 別可讓您存取HTL檔案中可用的相同上下文物件集(請參閱 [全域物件](global-objects.md))。

在擴展的類中，可 `WCMUsePojo`以通過名稱訪問上下文對象，使用
...
* 表單的方法將在HTL `getXyz` 檔案中公開一個名為的物件屬性 `xyz`。

在下列範例中，這些方 `getTitle` 法和 `getDescription` 結果會導致物件屬性 `title` ，並可 `description` 在HTL檔案的內容中存取：
...
此 `data-sly-use` 屬性用於初始化HTL程式碼中的use-class。 在我們的示例中， `data-sly-use` 屬性聲明我們要使用類 `Info`。 我們只能使用類的本地名稱，因為我們使用的是本地安裝（將Java源檔案放在與HTL檔案相同的資料夾中）。 如果我們使用套件安裝，我們必須指定完全限定的類別名稱。
...
識別 `info` 碼(位於中的點 `data-sly-use.info`後)用於HTL檔案中以識別類別。 此標識符的範圍在檔案中是全局的，在聲明後。 它不限於包含語句的元 `data-sly-use` 素。
...
參數是透過方法存 `WCMUsePojo` 取 [`<T> T get(String paramName, Class<T> type)`](https://helpx.adobe.com/experience-manager/6-2/sites/developing/using/reference-materials/javadoc/com/adobe/cq/sightly/WCMUse.html)
...
>[!NOTE]
>
>只有當use-class用於從另一個HTL檔案呼叫的檔案中，且 `data-sly-template` 其中包含需要傳遞的參數時，才應將參數傳遞至use-class。

例如，讓我們在現有範例旁 `data-sly-template` 建立個別檔案。 我們將調用新檔案 `extra.html`。 它包含一個 `data-sly-template` 名為 `extra`:
...
和，語 `data-sly-use` 句必須引用完全限定的類名，而不只是本地類名：
...
### 替代項目 `WCMUsePojo` {#alternatives-to-wcmusepojo}
...
* 如果與HTL檔案位 `UseClass.java` 於同一目錄中，請嘗試編譯並載入該類。 如果成功轉到(2)。
* 否則，請 `UseClass` 解譯為完全限定的類名，並嘗試從OSGi環境載入它。 如果成功轉到(2)。
* 否則，請 `UseClass` 解譯為HTL或JavaScript檔案的路徑，並載入該檔案。 如果goto(4)成功，

(2)

* 嘗試讓電流適 `Resource` 應 `UseClass`。 如果成功，請轉至(3)。
...
* 如果 `UseClass` 實 [`io.sightly.java.api.Use`](https://helpx.adobe.com/experience-manager/6-2/sites/developing/using/reference-materials/javadoc/io/sightly/java/api/Use.html) 現，則調用方 `init` 法，傳遞當前執行上下文(以對象的形式 `javax.scripting.Bindings` )。
...
* 不過，如果use-class實 *作* ，則其方法會自動與目前的內容 `Use``init` 一起呼叫，讓您將初始化程式碼放在該內容上。
* 擴充的使用類別 `WCMUsePojo` 只是實作的特例 `Use`。 它提供了方便的上下文方法，並 `activate` 且自動調用其方法 `Use.init`。

### 直接實作介面使用 {#directly-implement-interface-use}

雖然建立使用類別最常見的方式是擴充 `WCMUsePojo`，但也可以直接實作介 [`io.sightly.java.api.Use`](https://helpx.adobe.com/experience-manager/6-2/sites/developing/using/reference-materials/javadoc/io/sightly/java/api/Use.html) 面本身。
...
所有其他功能(如等同的 `WCMUsePojo.getProperties()`)都必須使用物件明確實 [`javax.script.Bindings`](http://docs.oracle.com/javase/7/docs/api/javax/script/Bindings.html) 作。 例如：
...
若要從HTL中存取資產，您可以在 [`com.day.cq.dam.api.Asset`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/com/adobe/granite/asset/api/Asset.html) 陳述式中宣告為類別，然 `data-sly-use` 後使用get方法 `Asset` 擷取所需資訊。 例如：
...
此語 `data-sly-use` 句會指示HTL將目前版本 `Resource` 改 `Asset` 寫為本機名稱 `asset`。 然後，它會呼叫 `getMimeType` 使用HTL getter `Asset` 速記的方法： `asset.mimeType`.

### 自請求調整 {#adaptable-from-request}

也可將任何可從 [`org.apache.sling.api.SlingHttpServletRequest`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/SlingHttpServletRequest.html)

與上述適用於的use類的情況一樣， `Resource`可在語句中指定 [`SlingHttpServletRequest`](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/reference-materials/javadoc/org/apache/sling/api/SlingHttpServletRequest.html) 適用於的use類 `data-sly-use` 。 執行時，目前的要求將會適應給定的類別，而產生的物件將可在HTL中使用。
